 ...
 */
/**
 * Open the upstream stream and return its status WITHOUT piping yet. Lets the
 * caller check for 429/4xx first (and retry to another account) before it
 * commits a 200 SSE response to the client.
 * @returns {Promise<{status, headers, text?, errorCode?, stream?}>}
 *   On error (status>=400): {status, headers, text, errorCode}.
 *   On success: {status, headers, stream} — caller must pump it via pumpStream.
 */
export async function openStream(url, apiKey, body, { timeoutMs = 300000 } = {}) {
  const res = await fetch(url, {
    method: "POST",
    headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
    body: JSON.stringify(body),
