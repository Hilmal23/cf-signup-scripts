// SQLite via Node's built-in node:sqlite (Node 22+). Synchronous, no native build.
...
export function initDb(dbPath) {
  ...
  // Migration: neuron-tracking columns. node:sqlite has no ADD COLUMN IF NOT EXISTS,
  // so add each and ignore "duplicate column" — makes this idempotent on existing DBs.
  for (const col of [
