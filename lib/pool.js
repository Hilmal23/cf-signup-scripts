...
export class AccountPool {
  constructor(db, { cooldown429 = 90, log = console, reserveNeurons = 250 } = {}) {
    ...
    this._selAvail = db.prepare(
      // One row, round-robin via OFFSET. Fetching all candidates per call cost
      // ~11ms at 4753 accounts (node:sqlite .all() materializes every row);
      // LIMIT 1 OFFSET drops that to ~1ms. In-flight reservations still need a
      // JS re-check, so on a rare hit we advance and re-query — see getAvailable.
    ...
    // Count of eligible rows (for the modulo + OFFSET). Same predicate.
    this._countEligible = db.prepare(
      `SELECT COUNT(*) AS c FROM accounts
       WHERE is_active = 1 AND cooldown_until < ?
         AND (CASE WHEN neurons_day = ? THEN neurons_today ELSE 0 END) < ?`
    );
    // Same predicate, columns needed only for counting available (with reservations).
  ...
  getAvailable() {
    ...
    // Round-robin via OFFSET. An account whose COMMITTED budget is under the
    // cap but whose COMMITTED + IN-FLIGHT crosses it (a concurrent request just
    // reserved on it) is skipped — advance the cursor and try the next. In
    // practice this re-query rarely fires (reservations are small + transient).
    ...
    return null; // every eligible account is fully reserved in-flight
  }
  ...
  /**
   * Cooldown an account after a 429. Two distinct cases:
   *   - error code 4006 (daily neuron allocation exhausted): NO cooldown_until.
   *     Pin neurons_today to the cap — the budget filter (`neurons_today >=
   *     NEURON_FREE_DAILY`) already skips it, and the lazy day-rollover in
   *     getAvailable re-enables it automatically at 00:00 UTC. A cooldown_until
   *     here would just duplicate that mechanism.
   *   - other 429 (per-minute rate limit): the account still has its daily
   *     budget, so the budget filter WON'T skip it — set cooldown_until so the
   *     proxy avoids re-selecting it while it's still rate-limited.
   */
