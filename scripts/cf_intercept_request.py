#!/usr/bin/env python3
"""
CF Signup - REQUEST ROUTE INTERCEPTION (CRITICAL FIX!)
page.on("request") can't abort/modify requests.
Use page.route() to intercept AND abort, then send modified request.
"""
...
def signup(token):
    ...
    with sync_playwright() as p:
        ...
        # ROUTE INTERCEPTION - capture request then abort
        api_req_data = [None]
        ...
        def handle_route(route):
            ...
            if 'user/create' in req.url:
                ...
                body_str = req.post_data
                
                try:
                    body_data = json.loads(body_str or '{}')
                except:
                    body_data = {}
                ...
                log(f"  URL: {req.url}")
                log(f"  Body keys: {list(body_data.keys())}")
                route_intercepted[0] = True
                
                # ABORT this request - we'll send our own
                route.abort()
        ...
        time.sleep(3)
        log(f"\nIntercepted: {route_intercepted[0]}")
        ...
        if api_req_data[0] and route_intercepted[0]:
            ...
            log(f"\n=== Modified body with token ===")
            log(f"Body keys: {list(req_info['body'].keys())}")
            log(f"Token: {token[:50]}...")
            
            # Send modified request via requests with BD proxy
            ...
            log(f"Modified request status: {r.status_code}")
...
def main():
    log("=== CF Signup - Route Interception + Token Inject ===")
    token = solve()
    if token:
        signup(token)
    log("\nDone")
